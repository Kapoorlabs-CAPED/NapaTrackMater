...
class TrackMate(object):
    ...
    def _iterate_dividing_recursive(self, root_leaf, target_cell, sorted_root_splits, gen_count, tracklet_count, tracklet_count_taken):
                ...
                if next_target_cell is not None:
                    ...
                    if next_target_cell in self.edge_target_lookup:
                            ...
                            for k in range(len(target_cells)):
                                ...
                                tracklet_count = self._unique_tracklet_count(tracklet_count_taken, tracklet_count)
                                tracklet_count_taken.append(tracklet_count)
                                self._iterate_dividing_recursive(root_leaf, target_cell, sorted_root_splits, next_gen_count, tracklet_count, tracklet_count_taken)      
            ...
            for root_all in root_root:
                    ...
                    if root_all in self.edge_target_lookup and root_all in root_splits:
                         ...
                         for j in range(len(target_cells)):
                                ...
                                tracklet_count = self._unique_tracklet_count(tracklet_count_taken, tracklet_count)
                                tracklet_count_taken.append(tracklet_count)
                                self._iterate_dividing_recursive(root_leaf, target_cell, root_splits, gen_count, tracklet_count, tracklet_count_taken)
            ...
            if len(root_splits) > 0:
                    ...
                    if first_split in self.edge_target_lookup:
                        ...
                        for i in range(len(target_cells)):
                            ...
                            self._iterate_dividing_recursive(root_leaf, target_cell, sorted_root_splits, gen_count, tracklet_count, tracklet_count_taken)
    ...
    def _track_computer(self, track, track_id):
                            ...
                            for leaf in root_leaf:
                                   ...
                                   self.unique_spot_properties[leaf].update({self.displacement_key : track_displacement})
                                   self.unique_spot_properties[leaf].update({self.total_track_distance_key : total_track_distance})
                                   self.unique_spot_properties[leaf].update({self.max_distance_traveled_key : max_track_distance})
                                   self.unique_spot_properties[leaf].update({self.track_duration_key : track_duration})
                            ...
                            for source_id in all_source_ids:
                                        ...
                                        self.unique_spot_properties[source_id].update({self.displacement_key : track_displacement})
                                        self.unique_spot_properties[source_id].update({self.total_track_distance_key : total_track_distance})
                                        self.unique_spot_properties[source_id].update({self.max_distance_traveled_key : max_track_distance})
                                        self.unique_spot_properties[source_id].update({self.track_duration_key : track_duration})
    ...
    def _dict_update(self, unique_tracklet_ids: List,  cell_id: int, track_id: int, source_id: int, target_id: int):
        ...
        self.unique_spot_properties[int(cell_id)].update({self.eccentricity_comp_firstkey : -1})
        self.unique_spot_properties[int(cell_id)].update({self.eccentricity_comp_secondkey : -1})
